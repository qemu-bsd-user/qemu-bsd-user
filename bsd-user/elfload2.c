...
load_elf_sections(const struct elfhdr *hdr, struct elf_phdr *phdr, int fd,
                  abi_ulong rbase, abi_ulong *baddrp, struct image_info *info)
{
    ...
    fprintf(stderr, "func %p rbase %p fred = %p\n", load_elf_sections, &rbase, &fred);
    /*
     ...
     */
    first = true;
    fprintf(stderr, "Loading %d sections\n", hdr->e_phnum);
    for (i = 0; i < hdr->e_phnum; i++) {
        ...
        if (eppnt->p_type != PT_LOAD) {
            fprintf(stderr, "Skipping section %d type %#x\n", i, eppnt->p_type);
            continue;
        }
        fprintf(stderr, "section %d sections\n", i);
                    ...
                    struct image_info *info)
{
    ...
    error = load_elf_sections(&elf_ex, elf_phdata, bprm->fd, et_dyn_addr, &load_addr, info);
    for (i = 0, elf_ppnt = elf_phdata; i < elf_ex.e_phnum; i++, elf_ppnt++) {
        if (elf_ppnt->p_type != PT_LOAD) {
            continue;
        }

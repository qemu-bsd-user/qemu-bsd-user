 ...
 *
 * The linux-user version of this uses the top 16 bits, but FreeBSD's SI_USER and
 * other signal indepenent SI_ codes have bit 16 set, so we only use the top byte
 * instead.
 *
 * For FreeBSD, we have si_pid, si_uid, si_status, and si_addr always. Linux and
 * {Open,Net}BSD have a different approach (where their reason field is larger, but
 * whose siginfo has fewer fields always).
 */

...
static unsigned long init_guest_space(unsigned long host_start,
                    ...
                    struct image_info *info)
{
    ...
    for (i = 0, elf_ppnt = elf_phdata; i < elf_ex.e_phnum; i++, elf_ppnt++) {
        ...
        loaddr = MIN(elf_ppnt->p_vaddr - elf_ppnt->p_offset, loaddr);
        hiaddr = MAX(elf_ppnt->p_vaddr + elf_ppnt->p_memsz, hiaddr);
    }
    ...
    if (elf_ex.e_type == ET_DYN && baddr == 0) {
        ...
        probe_guest_base(bprm->filename, loaddr, hiaddr);
    }
